-- CRAVE: optional geo radius for recommend_restaurants_for_user (ST_DWithin on location_geog).
-- When lat/lng are provided and the geo-filtered query returns no rows, falls back to the same logic without geo.
