-- CRAVE: restaurant card fields (ratings, copy, distance hint) + recommend RPC exposes them and optional distance_meters when geo is active.

set search_path = public, extensions;

alter table public.restaurants
  add column if not exists star_rating numeric(3, 2);
...
alter table public.restaurants
  drop constraint if exists restaurants_star_rating_range;

alter table public.restaurants
  add constraint restaurants_star_rating_range check (
    star_rating is null
    or (
      star_rating >= 0::numeric
      and star_rating <= 5::numeric
    )
  );

alter table public.restaurants
  drop constraint if exists restaurants_review_count_nonneg;

alter table public.restaurants
  add constraint restaurants_review_count_nonneg check (
  ...
  if v_pref is not null then
    if v_use_geo then
      ...
      select
        ...
        r.distance_label,
        case
          when r.location_geog is not null then ST_Distance(
            r.location_geog,
