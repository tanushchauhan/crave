...
GEN_DIR="${AWS_ROOT}/.generated"
mkdir -p "$GEN_DIR"
BOOT_ENV="${GEN_DIR}/bootstrap.env"
cat >"$BOOT_ENV" <<EOF
# Generated by infra/aws/scripts/bootstrap.sh — source from deploy-aws-from-env.sh
export CRAVE_AWS_ACCOUNT_ID="${ACCOUNT_ID}"
export CRAVE_RECEIPTS_BUCKET="${RECEIPTS_BUCKET}"
export CRAVE_ASSETS_BUCKET="${ASSETS_BUCKET}"
export CRAVE_AWS_REGION="${AWS_REGION}"
EOF
echo "Wrote ${BOOT_ENV}"

echo "Next: run infra/aws/scripts/deploy-aws-from-env.sh (loads repo .env + applies IAM, Lambdas, API Gateway, S3 trigger)."
