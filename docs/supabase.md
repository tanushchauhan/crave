...
| **Edge Functions (Deno)** | Deployed names: **`resolve-group`**, **`recommend`**, **`place-order`**, **`match-receipt-items`**, optional **`generate-ad`** — map to voice tools `resolve_group`, `recommend_restaurants`, `confirm_booking` (client or Edge), `place_order` ([plan.md section 8](plan.md#8-24-hour-build-timeline)). |
