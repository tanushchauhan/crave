...
| ----------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Google Places API**                     | Restaurant name, location, cuisine tags, price tier, rating, photos, hours | **Production path:** best license-safe coverage, hours, photos, and ratings for a city-wide catalog. **Hackathon MVP:** we do *not* require a Places API key. Instead we seed ~50 real venues near UT from **OpenStreetMap (Overpass)** (no key) plus **10 hand-curated CRAVE partners** with menus; see [tools/supabase-seed/README.md](../tools/supabase-seed/README.md). |
| **Yelp Fusion API**                       | Review snippets, more granular category tags, menu hints                   | **Production path:** richer tags and review snippets when you have a Fusion API key. **Hackathon MVP:** skipped so onboarding stays zero-key; OSM tags + curated copy cover the demo.                                                                                          |
...
1. **Home / "Ask Crave!"** — Dashboard landing page. Prominent search-style input — _"Ask Crave anything about your restaurant…"_ Below it: a KPI strip backed by **`get_dashboard_kpis`** (today's bookings, covers, average party size, sentiment vs prior week). Then a unified **Live Bookings and Orders** table (item 2), a **Menu Performance** summary (item 4), and an **AI Trending Now** card. The trending card is **static placeholder copy** for the hackathon demo; the data-backed Trend Radar story is item 6.

2. **Live Bookings and Orders** — A unified real-time stream powered by Supabase Realtime that displays both incoming food orders and bookings placed through the mobile app. New rows animate in within ~500ms. It surfaces as a table with the following fields: Food Ordered, Phone No (that reservation was booked under), Notes (for the incoming party), Size (party size), Date, and Time.

3. **Live Menu Management** — A CRUD interface where partners can add, edit, price, and disable menu items in real-time, syncing instantly to the voice agent's context and the mobile visual menu.

4. **Menu Performance** — Per-item analytics driven primarily by receipt OCR feedback (§3.3). For each menu item: impression count (from consumer recommendation sessions), order rate (from receipts), thumbs-up rate, thumbs-down rate, sentiment trajectory over time. Sortable table with an inline sparkline per row. Items with mixed feedback surface a "what customers said" preview when expanded.

5. **Ad Campaign Studio** — Creative generation workspace (Multimodal track bait). Prompt box + optional reference image upload → Bedrock image gen → generated Instagram-ready carousel (3 variants) + generated caption + suggested hashtags + "what this'd look like in the feed" preview. Short video variant renders via Remotion + **ElevenLabs** narration. Saved campaigns go to `ad_campaigns`; assets land in S3 and serve through CloudFront.

6. **Trend Radar** — City-level and neighborhood-level food trends actively populated by an AI (Claude via Bedrock) that aggregates intelligence from context in our Supabase database about the restaurant. _"'Birria' is up 40% in search intent this month in East Austin."_ Read-only for MVP; valuable content for the restaurant marketing story.
...
- **If `restaurants.is_crave_partner = true`**: the app writes a `bookings` row with `source='partner_app'` and `status='confirmed'`. Supabase Realtime broadcasts the row to every dashboard client subscribed to `restaurant_id = X`. The Live Bookings and Orders feed animates the new entry in within half a second. The restaurant doesn't need to do anything — the booking just shows up, with party size, context tag, dietary notes, and requested time.
...
| --------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hour 0–2: Setup**                     | All                                                             | Monorepo live. Supabase project created, schema applied, pgvector enabled. AWS accounts linked, Bedrock model access requested, Lambda skeletons deployed. Expo mobile shell builds. Next.js dashboard shell builds. Run **`cd tools/supabase-seed && npm install && npm run seed`** — loads ~50 OSM restaurants near UT + 10 curated partners with menus, demo users/groups, bookings, orders, and feedback; **Bedrock Titan** 1536-d embeddings inline (no Google Places / Yelp keys for MVP).                                                                                                                                                                                                                                                             |
...
| ↳ B2B dashboard                         | Founder 4                                                       | Login / signup (restaurant account: email + password + restaurant name → **`register_restaurant_on_signup`**), home page with "Ask Crave" input, **data-backed** KPI strip (`get_dashboard_kpis`), live bookings/orders merge + Realtime, menu performance from Supabase (static **AI Trending Now** copy only), campaign studio page skeleton. **`/dashboard/menu`:** menu CRUD on `menu_items` for the signed-in owner's restaurant.                                                                                                                                              |
...
| ↳ Supabase RLS + realtime               | Founder 4                                                       | RLS policies on all B2B tables. Realtime channel for live bookings and orders and group voting.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
